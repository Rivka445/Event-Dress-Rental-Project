...
namespace EventDressRental.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
        ...
        // PUT api/<OrdersController>/5/status/2
        [Authorize(Roles = "Admin")]
        ...
        public async Task<IActionResult> UpdateStatusOrder(int orderId, int statusId)
        {
            ...
            if (!await _orderService.IsExistsOrderById(orderId))
                return NotFound();

            await _orderService.UpdateStatusOrder(orderId, statusId);
            return Ok();
        }

        // Legacy: PUT api/<OrdersController>/status/5
        [Authorize(Roles = "Admin")]
        [HttpPut("status/{statusId}")]
        public async Task<IActionResult> UpdateStatusOrderLegacy([FromBody] OrderDTO orderDto, int statusId)
        {
            if (!_orderService.checkStatus(statusId))
                return BadRequest("not status match");
            ...
            await _orderService.UpdateStatusOrder(orderDto.Id, statusId);
            return Ok();
        }
